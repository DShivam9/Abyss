...
import { CardObject, MediaPoolItem, MediaDef, AssetMetadata } from "./types";
import {
  CELL_SIZE,
  CARD_SIZE,
  FRAME_SIZE,
...
export interface CycloramaMatrixSceneOptions {
  canvas: HTMLCanvasElement;
  container: HTMLDivElement;
  fogVeil: HTMLDivElement | null;
  media: MediaDef[];
...
export interface CycloramaMatrixSceneHandle {
  updateProps: (props: {
    radiusX?: number;
    radiusY?: number;
    baseCamZ?: number;
...
export function createCycloramaMatrixScene(
  options: CycloramaMatrixSceneOptions
): CycloramaMatrixSceneHandle {
  const { canvas, container, fogVeil, media, metadata } = options;

  let radiusX = options.radiusX;
  let radiusY = options.radiusY;
  let baseCamZ = options.baseCamZ;
  let zoomCamZ = options.zoomCamZ;
  let friction = options.friction;
  let onCardClick = options.onCardClick;
  ...
  renderer.setPixelRatio(options.dpr ?? window.devicePixelRatio ?? 1);
  renderer.setSize(cachedW, cachedH);
  renderer.setClearColor(0x030305, 1);

  const scene = new THREE.Scene();
  const camera = new THREE.PerspectiveCamera(54, cachedW / cachedH, 0.1, 100);
  camera.position.set(0, 0, baseCamZ);
  camera.lookAt(0, 0, 0);

  const textureLoader = new THREE.TextureLoader();

  const mediaPool: MediaPoolItem[] = media.map((item) => {
  ...
  const resumeVideos = () => {
    mediaPool.forEach((m) => m.vid && m.vid.paused && m.vid.play().catch(() => {}));
  };
  ...
  const labelTextures = metadata.map((meta) => {
  ...
  const camZUniform = { value: baseCamZ };
  ...
  for (let c = 0; c < COLS; c++) {
    for (let r = 0; r < ROWS; r++) {
      ...
      const mediaItem = mediaPool[mediaIndex];

      const bgMat = new THREE.ShaderMaterial({
        vertexShader: bgVert,
        fragmentShader: bgFrag,
        uniforms: {
          uTexture: { value: mediaItem.texture },
          uCellCenter: { value: new THREE.Vector2() },
          uRadiusX: { value: radiusX },
          uRadiusY: { value: radiusY },
          uHover: { value: 0.0 },
          uAspect: { value: mediaItem.aspect },
          uCamZ: camZUniform,
          uIntro: introUniform
        },
        transparent: true,
      ...
      const mesh = new THREE.Mesh(mediaItem.geom, mat);
      mesh.renderOrder = 2;
      cardGroup.add(mesh);
  ...
  const onPointerUp = (e: PointerEvent) => {
    if (isDragging) {
      ...
      // Detect click if minimal drag movement
      const dist = Math.hypot(e.clientX - pointerDownX, e.clientY - pointerDownY);
      if (dist < 6 && hoveredCard && onCardClick) {
        onCardClick(hoveredCard.mediaIndex, metadata[hoveredCard.mediaIndex]);
      }
    }
  };
  ...
  function animate() {
    ...
    if (introActive) {
      ...
      if (introElapsed > INTRO_DELAY) {
        ...
        currentCamZ = THREE.MathUtils.lerp(baseCamZ + 4.2, baseCamZ, ease);
        ...
        if (fogVeil) {
          ...
          fogVeil.style.opacity = veilOpacity.toString();
        }
        ...
        if (t >= 1.0) {
          ...
          if (fogVeil) {
            fogVeil.style.display = "none";
          }
        }
      } else {
        currentCamZ = baseCamZ + 4.2;
        camera.position.z = currentCamZ;
        camZUniform.value = currentCamZ;
        panY = 0.95;
        introUniform.value = 0.0;
        if (fogVeil) {
          fogVeil.style.opacity = "1";
        }
      }
    } else {
      const targetCamZ = isDragging ? zoomCamZ : baseCamZ;
    ...
    if (hasMouse && !isDragging && !reducedMotion) {
    ...
    if (!introActive && hasMouse && !isDragging) {
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        const dY = Math.abs(mouseY - screenY);
    ...
    const zoomScale = Math.max(1.0, currentCamZ / baseCamZ);
  ...
  return {
    ...
    updateProps(newProps) {
      if (newProps.radiusX !== undefined) radiusX = newProps.radiusX;
      if (newProps.radiusY !== undefined) radiusY = newProps.radiusY;
      if (newProps.baseCamZ !== undefined) baseCamZ = newProps.baseCamZ;
      if (newProps.zoomCamZ !== undefined) zoomCamZ = newProps.zoomCamZ;
      if (newProps.friction !== undefined) friction = newProps.friction;
      if (newProps.onCardClick !== undefined) onCardClick = newProps.onCardClick;
      if (newProps.reducedMotion !== undefined) reducedMotion = newProps.reducedMotion;
