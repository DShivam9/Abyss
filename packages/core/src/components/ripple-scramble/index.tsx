...
const ALPHA_STR_TABLE: string[] = Array.from({ length: 101 }, (_, i) => (i / 100).toFixed(2));

// Pure helper function for distinct variant-specific design specifications
function getVariantSpecs(variant: ApparatusRippleVariant, staticOpacity: number) {
  ...
  switch (variant) {
    case "editorial":
      return {
        ...
        decayColor: (alphaStr: string) => `rgba(235, 235, 240, ${alphaStr})`,
    ...
    case "matrix":
      return {
        ...
        decayColor: (alphaStr: string) => `rgba(134, 239, 172, ${alphaStr})`,
        flareColor: (intensity: number) => {
    ...
    default:
      return {
        ...
        decayColor: (alphaStr: string) => `rgba(225, 225, 230, ${alphaStr})`,
        flareColor: (intensity: number) => {
